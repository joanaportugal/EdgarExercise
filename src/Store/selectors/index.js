...
export const getPokemonID = createSelector(getCurrentPage, page => {
  const url = page;
  if (!url) return null;
...
export const getPokemonById = createSelector(
  ...
  getPokemonID,
  (pokemons, id) => {
    const pokemonKey = Object.keys(pokemons).find(
      pokemon => pokemons[pokemon].id == id
    );
...
export const getAllPokemonsByType = createSelector(
  ...
  (selectedType, pokemons) => {
    ...
    Object.keys(pokemons).forEach(pokemonName => {
      if (
        pokemons[pokemonName].types.find(
...
export const getAllPokemonsOnList = createSelector(
  ...
  (pokemonsList, startId, endId) => {
    ...
    Object.keys(pokemonsList).forEach(pokemonName => {
      if (
        pokemonsList[pokemonName].id >= startId &&
        pokemonsList[pokemonName].id <= endId
      ) {
